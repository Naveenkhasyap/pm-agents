...
class GoogleNews:
    ...
    def fetch_search_documents(self, query: str, helper = True, when = None, from_ = None, to_ = None):
        results = self.search(query, helper, when, from_, to_)
        documents = []
        for i, val in enumerate(results['entries']):
            doc = Document(
                page_content=results['entries'][i]['title'],
                metadata={"source": "news", "domain":results['entries'][i]['source']['href']},
                id=results['entries'][i]['id'],
            )
